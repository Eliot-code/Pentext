...
_SIGNATURES: List[EDRSignature] = [
    EDRSignature(
        ...
        preferred_bypass='amsi_veh_patchless', preferred_sleep='ekko', preferred_gate='halos_gate',
    ),
    EDRSignature(
        ...
        paths=[r'C:\Program Files\CrowdStrike', '/opt/CrowdStrike', '/Library/CS',
        ...
        preferred_bypass='amsi_hwbp', preferred_sleep='zilean', preferred_gate='halos_gate',
    ),
    EDRSignature(
        ...
        paths=[r'C:\Program Files\SentinelOne', '/opt/sentinelone', '/Library/Sentinel'],
        env_vars=['S1_AGENT_ID'],
        registry=[r'HKLM\SOFTWARE\Sentinel Labs',
                  r'HKLM\SYSTEM\CurrentControlSet\Services\SentinelAgent'],
        preferred_bypass='amsi_scanbuffer_patch', preferred_sleep='deathsleep',
    ...
    EDRSignature(
        ...
        preferred_bypass='amsi_com_hijack', preferred_sleep='cronos', preferred_gate='halos_gate',
    ),
    EDRSignature(
        ...
        env_vars=[], registry=[r'HKLM\SOFTWARE\Cylance\Desktop'],
        preferred_bypass='amsi_unicode_split', preferred_sleep='foliage',
    ...
    EDRSignature(
        ...
        paths=[r'C:\Program Files\Sophos', '/opt/sophos-spl', '/Library/Sophos Anti-Virus'],
        env_vars=[], registry=[r'HKLM\SOFTWARE\Sophos'],
        preferred_bypass='amsi_unicode_split', preferred_sleep='foliage',
        preferred_gate='hells_gate',
    ),
    EDRSignature(
        ...
        env_vars=[], registry=[r'HKLM\SOFTWARE\TrendMicro'],
        preferred_bypass='amsi_initfailed_v1', preferred_sleep='ekko',
        preferred_gate='hells_gate',
    ),
    EDRSignature(
        ...
        paths=[r'C:\Program Files\ESET', '/opt/eset',
               '/Library/Application Support/ESET'],
        env_vars=[], registry=[r'HKLM\SOFTWARE\ESET'],
        preferred_bypass='amsi_unicode_split', preferred_sleep='foliage',
        preferred_gate='hells_gate',
    ),
    EDRSignature(
        ...
        env_vars=[], registry=[r'HKLM\SOFTWARE\Symantec'],
        preferred_bypass='amsi_initfailed_v1', preferred_sleep='ekko',
        preferred_gate='hells_gate',
    ),
    EDRSignature(
        ...
        env_vars=[], registry=[r'HKLM\SOFTWARE\Palo Alto Networks\Traps'],
        preferred_bypass='amsi_hwbp', preferred_sleep='zilean', preferred_gate='halos_gate',
    ),
    EDRSignature(
    ...
    EDRSignature(
        ...
        env_vars=[], registry=[],
        preferred_bypass='amsi_initfailed_v1', preferred_sleep='ekko', preferred_gate='',
    ),
    EDRSignature(
        name='osquery', vendor='osquery.io', tier=3,
        processes=['osqueryd', 'osqueryi'],
        services=['osqueryd'],
        paths=['/etc/osquery', r'C:\Program Files\osquery',
               '/usr/local/bin/osqueryd'],
        env_vars=[], registry=[],
        preferred_bypass='amsi_initfailed_v1', preferred_sleep='foliage', preferred_gate='',
    ),
]
    ...
    if sys_p == 'Linux':
        ...
        try:
            out = subprocess.check_output(['tasklist', '/fo', 'csv', '/nh'],
                stderr=subprocess.DEVNULL, timeout=10).decode('utf-8', 'ignore')
        ...
        try:
            out = subprocess.check_output(['ps', '-axo', 'comm'],
                stderr=subprocess.DEVNULL, timeout=10).decode('utf-8', 'ignore')
    ...
    if sys_p == 'Linux':
        ...
        try:
            out = subprocess.check_output(
                ...
                stderr=subprocess.DEVNULL, timeout=15).decode('utf-8', 'ignore')
...
class FingerprintResult:
    ...
    evasion_plan: Dict[str, str]
    tier:         int
    notes:        List[str] = field(default_factory=list)

    def as_dict(self) -> Dict[str, Any]:
        return {
            'detected': [{'name': s.name, 'vendor': s.vendor, 'tier': s.tier,
                          'evidence': self.evidence.get(s.name, [])} for s in self.detected],
...
class EDRFingerprinter:
    ...
    def fingerprint(self) -> FingerprintResult:
        ...
        for sig in _SIGNATURES:
            ...
            for p in sig.processes:
                if p.lower() in procs: hits.append(f'process:{p}')
            for s in sig.services:
                if s.lower() in services: hits.append(f'service:{s}')
            for path in sig.paths:
                if path in paths: hits.append(f'path:{path}')
            for key in sig.registry:
                if key in reg_keys: hits.append(f'registry:{key}')
            for var in sig.env_vars:
                if var in env_vars: hits.append(f'env:{var}')
        ...
        return FingerprintResult(detected=detected, evidence=evidence,
                                 evasion_plan=evasion_plan, tier=tier, notes=notes)

    @staticmethod
    def _build_evasion_plan(detected: List[EDRSignature]) -> Dict[str, str]:
        if not detected:
            return {'bypass': 'amsi_initfailed_v1', 'sleep_mask': 'ekko',
                    'syscall_gate': 'hells_gate', 'rationale': 'No EDR detected — minimal bypass selected'}
        primary = min(detected, key=lambda s: s.tier)
        gate = primary.preferred_gate or 'hells_gate'
        return {'bypass': primary.preferred_bypass, 'sleep_mask': primary.preferred_sleep,
                'syscall_gate': gate, 'primary_edr': primary.name,
                'rationale': (f'Strategy tuned for {primary.name} (tier {primary.tier}). '
    ...
    def _build_notes(detected: List[EDRSignature]) -> List[str]:
        ...
        if any(s.tier == 1 for s in detected):
            notes.append('Tier-1 EDR detected — kernel-level tamper protection likely. '
                         'Manual beacon opsec review recommended before deployment.')
        if len(detected) >= 2:
            notes.append(f'{len(detected)} overlapping security products — '
